...
//    }
    public List<ArticleHasTag> searchByCatId(int cat_id){
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = ConnectDB.getConnection();
            PreparedStatement ps = con.prepareStatement("select a.*,c.name, t.tag_id, s.value from articles a inner join categories c on a.categories_id= c.id inner join tags_has_articles t on a.id = t.article_id inner join tags s on t.tag_id= s.id where a.publish_date<= current_date() and a.categories_id=?");
            ps.setInt(1,cat_id);
            ResultSet rs = ps.executeQuery();
            while(rs.next())
            {
    ...
    public List<ArticleHasTag> getArticleAll(){
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = ConnectDB.getConnection();
            PreparedStatement ps = con.prepareStatement("select a.*,c.name, t.tag_id, s.value from articles a inner join categories c on a.categories_id= c.id inner join tags_has_articles t on a.id = t.article_id inner join tags s on t.tag_id= s.id where a.publish_date<= current_date()");
            ResultSet rs = ps.executeQuery();
            while(rs.next())
            {
                list.add(new ArticleHasTag(rs.getInt(1),
    ...
    public List<ArticleHasTag> getArticleByTagId(int tag_id){
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection con = ConnectDB.getConnection();
            PreparedStatement ps = con.prepareStatement("select a.*,c.name, t.tag_id, s.value from articles a inner join categories c on a.categories_id= c.id inner join tags_has_articles t on a.id = t.article_id inner join tags s on t.tag_id= s.id where a.publish_date<= current_date() and t.tag_id=?");
            ps.setInt(1,tag_id);
            ResultSet rs = ps.executeQuery();
            while(rs.next())
            {
